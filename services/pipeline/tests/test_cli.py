...
def test_print_source_registry_reports_nationwide_verification_state(
    ...
    assert output["summary"]["pending"] == 114
    assert output["summary"]["legal_hold"] == 104
    assert output["summary"]["source_not_found"] == 182
    assert output["summary"]["no_recent_data"] == 278
    ...
    assert output["summary"]["priority_group_counts"]["p1"]["legal_hold"] == 104
