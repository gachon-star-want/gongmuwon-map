...
def test_print_source_registry_reports_nationwide_verification_state(
    ...
    assert output["summary"]["verified_in_code"] == 512
    ...
    assert output["summary"]["pdf_vision_hold"] == 16
    assert output["summary"]["adapter_hold"] == 1047
    assert output["summary"]["invalid_source_pattern"] == 0
    assert output["summary"]["priority_group_counts"]["p1"]["total"] == 488
    assert output["summary"]["priority_group_counts"]["p1"]["verified_in_code"] == 169
