...
- 제주특별자치도청: 감사위원회 공식 업무추진비 공개 목록(`https://audit.jeju.go.kr/news/notice/open.htm`)과 상세·XLSX/HWP 다운로드 구조를 확인했다. 다만 감사위원회 전용 게시판으로 보여 도청 전체 업무추진비 통합 출처로 확정할 수 없고, 목록·상세 화면에서 공공누리 제1유형 또는 명확한 자유이용 표시가 확인되지 않으며 저작권보호정책에 상업적 목적 이용 불가가 명시되어 보류한다.
- 제주특별자치도의회: 공식 업무추진비공개 목록(`https://www.council.jeju.kr/clicknews/openpromotion.do`)과 상세·XLS/XLSX 다운로드 구조를 확인했다. 다만 목록·상세 화면에서 공공누리 제1유형 또는 명확한 자유이용 표시가 확인되지 않고 `ALL RIGHTS RESERVED` 저작권 문구만 확인되어 보류한다.
...
- 제주특별자치도청: 감사위원회 공식 업무추진비 공개 목록과 `page` 페이지네이션, 상세 화면 XLSX/HWP 다운로드 구조를 확인했다. 감사위원회 전용 게시판으로 보여 도청 전체 통합 출처로 확정할 수 없고, 제1유형 표시도 없으며 저작권보호정책에 상업적 목적 이용 불가가 명시되어 `legal_hold`로 등록했다.
- 제주특별자치도의회: 공식 업무추진비공개 목록과 `page` 페이지네이션, 상세 화면 XLS/XLSX 다운로드 구조를 확인했다. 공공누리 제1유형 또는 명확한 자유이용 표시가 확인되지 않고 `ALL RIGHTS RESERVED` 저작권 문구만 확인되어 `legal_hold`로 등록했다.
...
| 밀양시청 | `https://www.miryang.go.kr/twn/bbs/selectBoardList.do?bbsId=BBSMSTR_000000085910&mnNo=3040000&owd=sammun` | 공식 밀양시 읍면동 공개자료실 목록에서 2026년 월별 업무추진비 게시물이 확인되고, 상세가 공공누리 `출처표시` 조건으로 표시된다. 파일 서버는 브라우저 UA에서만 XLSX를 반환한다. 현재 코드는 우선 삼문동 보드를 verified source로 등록했고, 다른 부서·읍면동 보드는 후속 매핑이 필요하다. | XLSX | `attachment_board`, `pageIndex`, `followDetail=true`, `fileKinds=["xlsx"]`, browser UA | `verified_candidate` |
...
밀양시청 dry-run 결과: `--agency 밀양시청 --since 2026-01-01 --row-since 2026-01-01 --max-posts 5 --quality-mode warn`에서 5개 게시물, 21개 raw/normalized visits, 19개 place candidates, 15개 Kakao matches로 성공했다.
