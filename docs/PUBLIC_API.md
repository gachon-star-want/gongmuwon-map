...
쓰기성 엔드포인트는 Cloudflare Turnstile 토큰도 요구한다. 프론트엔드는 `turnstile_token`을 JSON body에 포함하고, API Route는 `TURNSTILE_SECRET_KEY`로 Siteverify를 호출한다. 서버 secret이 없거나 토큰 검증이 실패하면 요청은 DB 쓰기 전에 fail-closed 된다.
