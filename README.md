...
- **지도 중심(Map-first) 플로팅 검색 & 필터**: 데스크톱과 모바일 모두에서 상시 고정되는 사이드바 레이아웃을 걷어내고 지도가 화면 100%를 채우는 화면 구성. 상단 중앙에 플로팅 검색창(`MapSearchBar`)과 가로 스크롤형 필터 칩 바(`MapFilterBar`)를 제공합니다.
- **오버레이 목록 카드**: 검색 결과 리스트(`PlaceList`)를 우측 상단 팝업 시트 형태로 노출하며, `X` 버튼으로 간편하게 닫아 지도를 더욱 넓게 탐색할 수 있습니다.
- **프론트엔드 및 API 트래픽 최적화**:
  - 클라이언트 측 GET 요청 캐싱 및 동일한 동시 요청을 하나로 합치는 Request Deduplication 기능 적용.
  - 지도의 bbox(지도 영역) 좌표를 소수점 3자리로 양자화(quantization)하여 CDN 캐시 키 재사용 및 히트율 극대화.
  - 1글자 검색어 입력 시 원격 API 요청을 생략하고 로컬 필터링으로 매끄럽게 대체하여 서버 부하 차단.
  - Regions(24시간), Places(5분/30분) 등 API 성격별 최적화된 Vercel CDN `Cache-Control` 헤더 튜닝.
- Kakao Map 기반 식당 지도
